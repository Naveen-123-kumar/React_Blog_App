...
export default function FitnessComponent3() {
  return (
    <>
<Cards></Cards>
...
const Cards = () => {
    const { fitness } = useContext(NewsContext);
    const listitems = fitness?.slice(0, 3).map((card) => (
      <Grid item lg="12" xs={12}>
        <Link to={`/fittness/${card.id}`} style={{ textDecoration: "none" }}>
          <Paper elevation={1}>
            <Box
              sx={{
                width: { xs: "100%", md: "100%" },
